...
describe('Organisation Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call AgentType query and add missing value', () => {
      const organisation: IOrganisation = { id: 7272 };
      const agentType: IAgentType = { id: 24386 };
      organisation.agentType = agentType;

      const agentTypeCollection: IAgentType[] = [{ id: 24386 }];
      jest.spyOn(agentTypeService, 'query').mockReturnValue(of(new HttpResponse({ body: agentTypeCollection })));
      const additionalAgentTypes = [agentType];
      const expectedCollection: IAgentType[] = [...additionalAgentTypes, ...agentTypeCollection];
      jest.spyOn(agentTypeService, 'addAgentTypeToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ organisation });
      comp.ngOnInit();

      expect(agentTypeService.query).toHaveBeenCalled();
      expect(agentTypeService.addAgentTypeToCollectionIfMissing).toHaveBeenCalledWith(
        agentTypeCollection,
        ...additionalAgentTypes.map(expect.objectContaining),
      );
      expect(comp.agentTypesSharedCollection).toEqual(expectedCollection);
    });
    ...
    it('Should update editForm', () => {
      ...
      expect(comp.agentTypesSharedCollection).toContainEqual(agentType);
  ...
  describe('Compare relationships', () => {
    describe('compareAgentType', () => {
      it('Should forward to agentTypeService', () => {
        const entity = { id: 24386 };
        const entity2 = { id: 16861 };
        jest.spyOn(agentTypeService, 'compareAgentType');
        comp.compareAgentType(entity, entity2);
        expect(agentTypeService.compareAgentType).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
